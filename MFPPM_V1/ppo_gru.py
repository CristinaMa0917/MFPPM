...
restore = True
round = 2
TIMESTAMP ='1217' #TIMESTAMP = datetime.now().strftime("%Y%m%d-%H%M%S")
SUMMARY_DIR = os.path.join(OUTPUT_RESULTS_DIR, ENVIRONMENT, 'RPPO',TIMESTAMP)
MODEL_PATH = SUMMARY_DIR +'/model.ckpt'
LOG_DIR = os.path.join(OUTPUT_RESULTS_DIR,'records')
log_file = open(LOG_DIR+'/rppo_%s_%dk_%d.txt'%(ENVIRONMENT,EP_MAX/1000,round),'w')
...
class PPO(object):
    ...
    def _build_anet(self, name, trainable):
        ...
        with tf.variable_scope(name):
            a_lstm = tf.nn.rnn_cell.GRUCell(num_units=CELL_SIZE) #a_lstm = tf.nn.rnn_cell.BasicLSTMCell(num_units=CELL_SIZE)
    ...
    def _build_cnet(self, name, trainable):
        ...
        with tf.variable_scope(name):
            c_lstm = tf.nn.rnn_cell.GRUCell(num_units=CELL_SIZE)
            ...
            vf = tf.layers.dense(c_cell_out, 1, trainable=trainable, kernel_regularizer=w_reg, name="vnet_out")
        params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=name)
        return vf, params
...
'''Remerber to restore ppo if it is second RUN'''
if restore == True:
    ppo.saver.restore(ppo.sess,MODEL_PATH)
    print('Restore model from %s'%MODEL_PATH)

print('Start training on ppo_gru %s total %d episode'%(ENVIRONMENT,EP_MAX))
