...
class Critic(object):
    def __init__(self, sess, state_dim, action_dim, learning_rate, gamma, t_replace_iter, a, a_):
        ...
        with tf.variable_scope('a_grad'):
            ...
            tf.summary.scalar('a_grads',self.a_grads)
...
if OUTPUT_GRAPH:
    summary_writer = tf.summary.FileWriter(PATH+'/logs', graph=sess.graph)

#summ = tf.Summary.Value(tag = 'episoide reward',simple_value = 0)
...
for i_episode in range(MAX_EPISODES):
    ...
    while True:
        ...
        if done:
            #print('GLOBAL_STEP:',GLOBAL_STEP.eval(sess))
            ...
            summary = tf.Summary(value=[
                tf.Summary.Value(tag="epoch reward", simple_value=running_r)
            ])
